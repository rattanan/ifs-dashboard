...
export default async function HomePage() {
  ...
  return <div className="px-3 py-3 sm:px-4 lg:px-5">
    ...
    <section className="mt-4">
      <div className="mb-3 flex flex-wrap items-end justify-between gap-2"><div><div className="flex items-center gap-2"><span className="grid size-8 place-items-center rounded-xl bg-violet-100 text-violet-700"><Sparkles className="size-4"/></span><h2 className="text-base font-bold text-[#17346b]">Business Insight</h2></div><p className="mt-1 text-[11px] text-slate-500">วิเคราะห์จาก metric สำคัญของแต่ละ Dashboard เพื่อช่วยจัดลำดับการตัดสินใจ</p></div><Badge className="rounded-full bg-gradient-to-r from-violet-600 to-blue-600 px-3 py-1 text-white">AI-ready insight</Badge></div>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">{dashboardDefinitions.map((dashboard, index) => { const Icon = dashboard.icon; const highlight = highlightsByDashboard.get(dashboard.slug); const span = index < 3 ? "xl:col-span-2" : "xl:col-span-3"; const mobileBalance = index === dashboardDefinitions.length - 1 ? "sm:col-span-2 xl:col-span-3" : ""; return <Link key={dashboard.href} href={dashboard.href} className={`group relative flex min-h-[340px] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_8px_30px_rgba(15,23,42,.06)] transition duration-300 hover:-translate-y-1 hover:border-slate-300 hover:shadow-[0_18px_45px_rgba(15,23,42,.12)] ${span} ${mobileBalance}`}>
